<template>
  <header
      class="h-[5rem] max-w-[66.68338rem] mx-auto bg-white flex items-center justify-between">
    <router-link to="/" class="cursor-pointer">
  <LabelLogo text-color="#1D2939" inner-color="#1D2939" outer-color="#FFFFFF" />
    </router-link>
  <nav>
      ...
      in menuItems" :key="item.id">
        <button @click="handleDisplayCategories" v-if="item.id === 'a0a0a0a0'"
          ...
          <svg :class="{'transform rotate-180 transition-all duration-300 ease-linear':
                        displayCategories,
                        'rotate-0 transition-all duration-300 ease-linear': !displayCategories
                        }"
